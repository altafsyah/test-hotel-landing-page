import { useState } from "react";
import { Button } from "./ui/button";
import { ImageWithFallback } from "./figma/ImageWithFallback";
import { Calendar, ChevronDown, MapPin, Users } from "lucide-react";
import { DateRangeField, GuestsField, type Guests } from "./booking-fields";
import type { DateRange } from "react-day-picker";

export function Hero() {
  const [dateRange, setDateRange] = useState<DateRange | undefined>();
  const [guests, setGuests] = useState<Guests>({ adults: 2, children: 0 });

  return (
    <section className="relative h-screen w-full overflow-hidden">
      <ImageWithFallback
        src="/images/hero-1.jpg"
        alt="Azure Bay Resort"
        className="absolute inset-0 h-full w-full object-cover"
      />
      <div className="absolute inset-0 bg-gradient-to-b from-transparent from-30% to-black/50 to-70%" />
      <div className="px-2 pb-2 absolute bottom-0 left-0 flex flex-col items-center container mx-auto">
        <h1 className="font-normal text-white text-center">
          The Silence of the Alps, Redefined.
        </h1>
        <div className="bg-[#25262626]/15 ring ring-white/15 w-full mt-4 rounded-xl overflow-hidden backdrop-blur-3xl text-white">
          <div className="flex relative justify-between items-center p-4">
            <Users className="shrink-0 size-6" />
            <div className="grow mx-4">
              <span className="opacity-70">Guest</span>
              <p>Number of guests</p>
            </div>
            <ChevronDown className="shrink-0 size-6" />
          </div>
          <div className="flex relative justify-between items-center p-4">
            <Calendar className="shrink-0 size-6" />
            <div className="grow mx-4">
              <span className="opacity-70">Arrival & Departure</span>
              <p>Select date</p>
            </div>
            <ChevronDown className="shrink-0 size-6" />
          </div>
          <button className="bg-[#A49781] w-full py-3 px-8 text-white">
            REQUEST
          </button>
        </div>
      </div>
    </section>
  );
}
